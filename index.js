...
let botonNuevoJuegoTimeout = document.getElementById(
  "boton-nuevo-juego-timeout"
);
...
const dibujarGrillaHTML = (dimensiones) => {
  const { width } = grilla.getBoundingClientRect();
  const tamanio = width / dimensiones;

  grilla.innerHTML = "";

  for (let i = 0; i < dimensiones; i++) {
    for (let j = 0; j < dimensiones; j++) {
      grilla.innerHTML += `<div id="grilla" style="width:${tamanio}; height:${tamanio}px;  top: ${
        tamanio * i
      }px; left: ${tamanio * j}px;"  data-fila=${i} data-columna=${j}>
                ${matriz[i][j]} 
                </div>`;
    }
  }
